...
{
    if (!table_exists($table) || in_array($column, table_columns($table), true)) {
        return;
    }
...
{
    dapodik_ensure_tracking_columns();
    $name = dapodik_row_value($row, ['nama', 'nama_ptk', 'nama_guru']);
    if ($name === '') {
        return false;
    }

    $dapodikId = dapodik_external_id($row, ['ptk_id', 'guru_id', 'id_ptk', 'id_guru']);
    $nip = dapodik_row_value($row, ['nip']);
    $nuptk = dapodik_row_value($row, ['nuptk']);
    $gender = dapodik_row_value($row, ['jenis_kelamin', 'jk']);
    $position = dapodik_row_value($row, ['jenis_ptk', 'jabatan', 'tugas_tambahan']);
...
function dapodik_import_subject(array $row, bool $requireTeacher = true): ?int
{
    ...
    $dapodikId = dapodik_external_id($row, ['mata_pelajaran_id', 'mapel_id', 'id_mapel', 'id_mata_pelajaran']);
    $short = dapodik_subject_short_from_row($row, $name);
    $group = dapodik_row_value($row, ['kelompok', 'kelompok_mapel', 'group_name']);
...
{
    dapodik_ensure_tracking_columns();
    $name = dapodik_row_value($row, ['nama', 'nama_rombel', 'rombongan_belajar', 'rombel', 'nama_kelas', 'kelas']);
    if ($name === '') {
        return false;
    }
    ...
    $dapodikId = dapodik_external_id($row, ['rombongan_belajar_id', 'rombel_id', 'id_rombel', 'id_rombongan_belajar', 'rombongan_belajar_id_str']);
    $major = dapodik_row_value($row, ['nama_jurusan_sp', 'jurusan', 'program_keahlian', 'kompetensi_keahlian', 'major']);
    $academicYear = dapodik_academic_year_from_row($row);
...
function dapodik_import_student(array $row): bool
{
    ...
    $dapodikId = dapodik_external_id($row, ['peserta_didik_id', 'pd_id', 'id_pd', 'id_peserta_didik']);
    $nis = dapodik_row_value($row, ['nis', 'nipd', 'nomor_induk']);
    $nisn = dapodik_row_value($row, ['nisn']);
    $gender = dapodik_row_value($row, ['jenis_kelamin', 'jk']);
    $birthPlace = dapodik_row_value($row, ['tempat_lahir']);
    $birthDate = dapodik_row_value($row, ['tanggal_lahir', 'tgl_lahir']);
    $religion = dapodik_row_value($row, ['agama', 'agama_id_str']);
    ...
    if ($existingId) {
        execute_sql(
            ...
            [$dapodikId, $nis, $nisn, $name, $gender, $birthPlace, $birthDate !== '' ? $birthDate : null, $religion, $classId, now_string(), $existingId]
        );
        return true;
    }
    ...
    execute_sql(
        ...
        [$dapodikId, $nis, $nisn, $name, $gender, $birthPlace, $birthDate !== '' ? $birthDate : null, $religion, $classId, now_string()]
    );
...
function dapodik_import_pembelajaran(array $row): bool
{
    ...
    $dapodikId = dapodik_external_id($row, ['pembelajaran_id', 'id_pembelajaran']);
    $academicYear = dapodik_academic_year_from_row($row);
